...
RunSeuratBasics <-function(x) {
  ...
  y <- ScaleData(y)
  y <- RunPCA(y)
  y <- RunTSNE(y, dims = 1:50, num_threads = 16, check_duplicates = F)
  y <- RunUMAP(y, dims = 1:50)
...
pred_cellidc <- function(x, ref_seurat){
  ...
  cell_gs <- GetCellGeneSet(ref_seurat,dims = 1:50)
  x <- RunMCA(x,  nmcs = 50)
...
pred_cellidg <- function(x, ref_seurat){
  ...
  ref_seurat <- RunMCA(ref_seurat, nmcs = 50)
  x <- RunMCA(x,  nmcs = 50)
  group_gs <- GetGroupGeneSet(ref_seurat,dims = 1:50)
...
pred_SCN <- function(x, ref_seurat) {
  ...
  stList <- splitCommon(sampTab = stTM[stTM$cell_type1!="unknown",],  dLevel = "cell_type1")
  stTrain <- stList[[1]]
...
pred_MNN <- function(x, ref_seurat) {
  ...
  MNN <-
    findMutualNN(
      ...
      k1 = 50,
      k2 = 50,
      BPPARAM = BiocParallel::MulticoreParam(workers = 50,progressbar = T) 
    )
  MNN_pair <- dplyr::arrange(as.data.frame(MNN), second)
  MNN_pair$cell_type1 <-
    ref_seurat@active.ident[MNN_pair$second]
  MNN_prediction <-
    summarise(group_by(MNN_pair, first), prediction = names(which.max(table(cell_type1))))
  MNN_prediction$cell <-
    names(x@active.ident[MNN_prediction$first])
  MNN_prediction <-
...
pred_scPred <- function(x, ref_seurat) {
  ...
  scPred::metadata(scp) <-
    set_colnames(as.data.frame(ref_seurat@active.ident), "cell_type1")
...
pred_Seurat_LSI <- function(x, ref_seurat){
  set.seed(1)
  x <- RunLSI(x, n = 50, scale.max = NULL)
  transfer.anchors <- FindTransferAnchors(reference = ref_seurat, query = x, features = VariableFeatures(object = ref_seurat), 
                                          reference.assay = "RNA", query.assay = "RNA", reduction = "cca")
  prediction <- TransferData(anchorset = transfer.anchors, refdata = ref_seurat@meta.data$cell_type1, 
               weight.reduction = x[["lsi"]])
  prediction_vec <- ifelse(prediction$prediction.score.max > 0.5, prediction$predicted.id, "unassigned")
  pred_Seurat <- prediction_vec
  return(pred_Seurat)
}
