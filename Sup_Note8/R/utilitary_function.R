...
pred_cellidc <- function(x, ref_seurat){
  ...
  x <- RunMCA(x,  nmcs = 50)
  HGT_cells <- RunCellHGT(x, pathways = cell_gs, n.features = 200)
...
pred_cellidg <- function(x, ref_seurat){
  ...
  ref_seurat <- RunMCA(ref_seurat, nmcs = 50)
  x <- RunMCA(x,  nmcs = 50)
  group_gs <- GetGroupGeneSet(ref_seurat,dims = 1:50, group.by = "cell_type1")
  HGT_groups <- RunCellHGT(x, pathways = group_gs, n.features = 200, reduction = "mca")
...
pred_MNN <- function(x, ref_seurat) {
  ...
  integration <-
    intersect(VariableFeatures(FindVariableFeatures(ref_seurat)) , VariableFeatures(FindVariableFeatures(x)))
