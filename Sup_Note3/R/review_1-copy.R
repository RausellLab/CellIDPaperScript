...
use_python("/home/acortal/.conda/envs/r-reticulate/bin/python")
...
# -------------------------------------------------------------------------
x <- cite_seurat
run_biplots <- function(x, scaling, name = "biplots"){
  test_matrix <- as.matrix(t(x@assays$RNA@data))
  test_matrix_centered <- scale(test_matrix, center = TRUE, scale = T) 
  ...
  T <- (Q_sqrt) %*% X
  T_SVD <- irlba::irlba(T, nv = 50, nu = 50) # small matrix
  # T = U D_sing V'
  ...
  rownames(Std_Row_coordiantes) <- colnames(cite_seurat2@assays$RNA@data)
  rownames(Ppal_Col_coordiantes) <- rownames(cite_seurat2@assays$RNA@data)
  # The row-metric preserving biplot is given by:
  x@reductions[[name]] <- x@reductions$mca
  x@reductions[[name]]@cell.embeddings <- Std_Row_coordiantes
  x@reductions[[name]]@feature.loadings <- Ppal_Col_coordiantes
  return(x)
}
...
x <- NULL
cite_seurat2 <- run_biplots(cite_seurat, scaling =T, name = "scaled_biplots")
cite_seurat2 <- run_biplots(cite_seurat2, scaling = F, name = "centered_biplots")


DimPlot(cite_seurat2, reduction = "scaled_biplots")

plot(cite_seurat2@reductions$scaled_biplots@cell.embeddings[,c(3,4)])
points(cite_seurat2@reductions$scaled_biplots@feature.loadings[,c(3:4)], col = "red")
plot(cite_seurat2@reductions$mca@cell.embeddings[,1:2])

HGT_centered_biplots <- RunCellHGT(cite_seurat2, pathways = immune_signature, reduction = "centered_biplots")
HGT_scaled_biplots <- RunCellHGT(cite_seurat2, pathways = immune_signature, reduction = "scaled_biplots")

pm <- GetGroupGeneSet(cite_seurat2, reduction = "centered_biplots")
pm2 <- GetGroupGeneSet(cite_seurat2, reduction = "mca")
pm3 <- GetGroupGeneSet(cite_seurat2, reduction = "mca")
what <- rownames(HGT_centered_biplots)[apply(HGT_centered_biplots,2, which.max)]
ok <- ifelse(apply(HGT_centered_biplots,2, max)>2, what, "unassigned")

A <- rownames(HGT_scaled_biplots)[apply(HGT_scaled_biplots,2, which.max)]
ok <- ifelse(apply(HGT_scaled_biplots,2, max)>2,A, "unassigned")
table(ok, cite_seurat2@active.ident)
table(cite_seurat2@active.ident)

GetGroupGeneSet(cite_seurat2@reductions, reduction = "scaled_biplots")


cite_seurat2$pred_AUCell <- ok
DimPlot(cite_seurat2, group.by = "pred_CellID")

ImmuneBenchCell <-
  foreach(
    Seurat = list(cite_seurat2, subset(
      reap_seurat, idents = "unknown", invert = T
    )),
    mapping = list(map_cite, map_reap),
    data = c("CITE", "REAP"),
